...
class TestKitsuneChatbot:
    ...
    @pytest.mark.asyncio
    async def test_get_sql_from_model(self):
        fake_kitsune_db = MagicMock()
        fake_kitsune_db.run_sql_query = AsyncMock()
        fake_kitsune_db.run_sql_query.return_value = "Results:\ncol1: val1, col2: val2"
        ...
        fake_sql_agent.run.return_value = "sql query from agent"
        ...
        result = await chatbot._get_sql_from_model(question="any question")
        
        fake_lance_db.search.assert_called_once_with(query="any question")
        fake_sql_agent.run.assert_called_once_with(question="any question", context={"example": context_result})
        
        assert result == (context_result, "sql query from agent")
