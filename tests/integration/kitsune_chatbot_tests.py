...
class TestKitsuneChatbot:
    ...
    @pytest.mark.asyncio
    async def test_answer_without_session_id(self):
        fake_logger = MagicMock()
        chatbot = KitsuneChatbot(
            cache=build_cache_adapter(),
            guardrail=build_guardrail_adapter(),
    ...
    @pytest.mark.asyncio
    async def test_answer_with_last_session_id(self):
        fake_logger = MagicMock()
        chatbot = KitsuneChatbot(
            cache=build_cache_adapter(),
            guardrail=build_guardrail_adapter(),
        ...
        response, session_id = await chatbot2.answer(
            "How many policies do I have?",
            user_id=self.test_user_id,
            session_id=user_session_id,
        )
