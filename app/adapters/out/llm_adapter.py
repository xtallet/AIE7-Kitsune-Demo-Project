...
class LLMAdapter(LLM):
    ...
    async def summarize_answer(self, user_question: str, query_result: Any) -> str:
        """Summarize the answer in natural language based on the user's question and the query result."""
        system_prompt = (
        ...
        user_prompt = (
            ...
            "Provide a one-sentence answer that addresses the user's question."
        )
        try:
            # self.logger.info("Summarizing answer for user question: %s", user_question)
            self.logger.debug("Query result provided: %s", query_result)
            ...
            self.logger.debug("Generated summary: %s", summary)
            return summary
        except Exception as e:
            self.logger.error(
                "Failed to summarize answer for question: %s, Error: %s",
                user_question,
                str(e),
            )
            raise RuntimeError("Failed to summarize answer.") from e
